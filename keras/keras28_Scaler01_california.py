...
# print("x 데이터 : " , x)
# print(np.min(x), np.max(x)) # -124.35 35682.0
...
# 그런데 train/test를 나누기 전에 scaler.fit_transform(x)를 해버리면 scaler가 이미
# 전체 데이터 최소값 = 10, 전체 데이터 최대값 = 100 을 알아버립니다.
# 즉, 학습 과정에서 원래는 모르면 되는 test 데이터의 정보까지 미리 본 것이에요.
...
# train 데이터로 MinMaxScaler의 기준을 학습하고, test 데이터는 그 기준으로만 변환한다. 
# 이유는 test 정보가 학습 과정에 들어가는 데이터 누수를 막기 위해서다.
