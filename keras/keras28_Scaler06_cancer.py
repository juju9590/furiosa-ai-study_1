...
# 마지막은 무조건 sigmoid 1개만 쓴다. (activation 값 명시 안하면 디폴트값은 liner)
# activation = 활성화 함수, 한정함수 (음수가 나왔을때 relu 적용하면 양수가 됨)
# 시그모이드 함수는 + 무한대에서 1에 가까워지거나 - 무한대에서는 0에 가까워진다
# ㄴ(쏠레는) 텐서플로우에서 시그모이드는 반올림하여 0과 1로 나타내 준다. (0.5보다 크면 1, 작으면 0으로 간주)
# y_pred = sigmoid(wx+b)로 간주
...
model.compile(loss='binary_crossentropy', #(필수)이진분류모델
              ...
              metrics=['accuracy'], 
                # ㄴ 주요 보조지표 : ['acc']로 적어도 나옴, 리스트 형태로 나중에 보조지표 여러개 더 적을 수 있음
              ) 
# mse는 수치화된 오류
# 이진 분류에서는 "binary_crossentropy" 딱 하나 ( loss는 순전파, 역전파에서 가중치 갱신 하는 값 )
